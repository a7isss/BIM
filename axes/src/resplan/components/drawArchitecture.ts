        ...
        if (item.type === 'wall' && item.n1 && item.n2) {
            ...
            if (n1 && n2) {
                ...
                archGroup.append('line')
                    ...
                    .style('cursor', activeTool === 'delete' || activeTool === 'split_wall' || activeTool === 'add_door' || activeTool === 'add_window' ? 'pointer' : 'default')
                    .on('click', (e: any) => {
                        if (activeTool === 'delete') {
                            e.stopPropagation();
                    ...
                    .on('mouseenter', function() {
                        if (activeTool === 'delete' || activeTool === 'split_wall') {
                            d3.select(this).attr('stroke', activeTool === 'delete' ? 'rgba(239, 68, 68, 0.5)' : 'rgba(59, 130, 246, 0.5)');
                        }
                    })
...
const drawOpenings = (
    ...
    renderOpenings.forEach((op: any) => {
        ...
        const opGroup = archGroup.append('g')
            .style('cursor', activeTool === 'delete' || activeTool === 'move' || activeTool === 'rotate' ? 'pointer' : 'default')
            .on('click', (e: any) => {
                if (activeTool === 'delete') {
                    e.stopPropagation();
                    ...
                    e.stopPropagation();
                    setSelectedEntity({ id: op.id, type: op.type as 'door' | 'window' });
        ...
        if (selectedEntity && selectedEntity.id === op.id) {
            opGroup.append('rect')
...
export const drawArchitecture = (
    ...
    if (isEditMode && (activeTool === 'move' || activeTool === 'delete')) {
        const nodeGroup = archGroup.append('g').attr('class', 'architecture-nodes');
        nodes.forEach(node => {
            const isSelected = selectedEntity?.id === node.id && selectedEntity?.type === 'node';
            nodeGroup.append('circle')
                .attr('cx', toPxX(node.x))
                .attr('cy', toPxY(node.y))
                .attr('r', 10)
                .attr('fill', activeTool === 'delete' ? 'rgba(239, 68, 68, 0.6)' : (isSelected ? 'rgba(59, 130, 246, 0.8)' : 'rgba(59, 130, 246, 0.4)'))
                .attr('stroke', activeTool === 'delete' ? '#ef4444' : '#3b82f6')
                ...
                .on('click', (e: any) => {
                    ...
                    if (activeTool === 'delete') {
                        ...
                        setSelectedEntity({ id: node.id, type: 'node' });
                    }
                })
                .on('mouseenter', function() {
                    d3.select(this).attr('r', 14).attr('fill', activeTool === 'delete' ? 'rgba(239, 68, 68, 0.8)' : 'rgba(59, 130, 246, 0.8)');
                })
                .on('mouseleave', function() {
                    d3.select(this).attr('r', 10).attr('fill', activeTool === 'delete' ? 'rgba(239, 68, 68, 0.6)' : (isSelected ? 'rgba(59, 130, 246, 0.8)' : 'rgba(59, 130, 246, 0.4)'));
                });
        });
    }
    ...
    if (renderOpenings.length > 0) {
        drawOpenings(archGroup, renderOpenings, openings, types, toPxX, toPxY, pxPerMeter, activeTool, scope, selectedEntity, setSelectedEntity, updateState);
    }
};
